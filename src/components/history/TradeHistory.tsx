...
import { Search, Download, ChevronDown, ChevronLeft, ChevronRight, X, AlertCircle, XCircle, Eye, BarChart2, Filter, RefreshCw, ArrowUpRight, ArrowDownLeft } from "lucide-react"; 
...
export function TradesTab({ state }: { state: HistoryDemoState }) { 
  ...
  const filtered = useMemo(() => { 
    return TRADES.filter(t => { 
      ...
      if (query) { 
        ...
        return t.id.toLowerCase().includes(q) || t.zone.toLowerCase().includes(q) || t.slot.includes(q); 
      } 
      return true; 
    }); 
  }, [query, sideF, statusF]); 
  ...
  if (state === "error") return ( 
    <EmptyState icon={XCircle} title="Failed to load trades" description="Trade history could not be retrieved. Try again." action={ 
      <Button variant="secondary" size="sm" onClick={() => toast.info("Retrying…")}><RefreshCw size={12} />Retry</Button> 
    } /> 
  ); 
  if (state === "empty") return ( 
    <EmptyState icon={BarChart2} title="No trades yet" description="Your executed trades will appear here once you have placed and matched orders on the GridX market." /> 
  ); 
    ...
    <div className="space-y-3"> 
      ...
      <div className="flex flex-wrap gap-2"> 
        <div className="relative flex-1 min-w-[160px]"> 
          ...
          <input value={query} onChange={e => { setQuery(e.target.value); setPage(1); }} placeholder="Search ID, zone, slot…" 
        ...
        {[ 
          ...
          { val:statusF, set:(v:string)=>{setStatusF(v);setPage(1);}, opts:[{v:"all",l:"All status"},{v:"settled",l:"Settled"},{v:"pending",l:"Pending"},{v:"failed",l:"Failed"}] }, 
        ].map((f, fi) => ( 
      ...
      {filtered.length === 0 ? ( 
        <EmptyState icon={Filter} title="No results" description="Adjust your filters or search term." /> 
      ) : ( 
        <> 
          {/* Desktop table */} 
              ...
              <table className="w-full text-xs"> 
                <thead> 
                  <tr className="border-b border-gx-line bg-secondary/60"> 
                    {["Time","ID","Side","Slot","Energy","Base","Fee","Eff. price","Total","Zone","Status",""].map(h => ( 
                ...
                <tbody className="divide-y divide-gx-line"> 
                  {paged.map(t => ( 
                    <tr key={t.id} className="hover:bg-secondary/40 transition-colors"> 
                      <td className="px-3 py-3 text-gx-fg3 whitespace-nowrap">{t.executedAt}</td> 
                      <td className="px-3 py-3 font-mono font-semibold text-foreground">{t.id}</td> 
                      <td className="px-3 py-3"><SidePill side={t.side} /></td> 
                      <td className="px-3 py-3 font-mono text-gx-fg2 whitespace-nowrap">{t.slot}</td> 
                      <td className="px-3 py-3 font-mono text-foreground">{t.kWh}</td> 
                      <td className="px-3 py-3 font-mono text-foreground">{t.basePrice.toFixed(3)}</td> 
                      <td className="px-3 py-3 font-mono text-gx-fg3">{t.gridFee.toFixed(3)}</td> 
                      <td className="px-3 py-3 font-mono text-foreground">{t.effectivePrice.toFixed(3)}</td> 
                      <td className="px-3 py-3 font-mono font-semibold text-foreground">{t.totalAed.toFixed(3)}</td> 
                      <td className="px-3 py-3 text-gx-fg3 max-w-[120px] truncate">{t.zone}</td> 
                      <td className="px-3 py-3"><StatusPill {...TRADE_STATUS[t.status]} /></td> 
                      <td className="px-3 py-3"> 
                        <button onClick={() => setSelected(t)} className="p-1.5 rounded-lg text-gx-fg4 hover:text-foreground hover:bg-secondary transition-all"> 
                          <Eye size={13} /> 
                        </button> 
                      </td> 
                    </tr> 
            ...
            {paged.map(t => ( 
              <button key={t.id} onClick={() => setSelected(t)} 
                ...
                <div className="flex items-center justify-between text-xs"> 
                  <span className="text-gx-fg3">{t.slot} · {t.executedAt}</span> 
                  <span className="font-mono font-bold text-foreground">AED {t.totalAed.toFixed(3)}</span> 
                </div> 
                <div className="flex items-center justify-between text-xs mt-1"> 
                  <span className="text-gx-fg4">{t.kWh} kWh · AED {t.effectivePrice.toFixed(3)}/kWh</span> 
                  <span className="text-gx-fg4 truncate max-w-[120px]">{t.zone}</span> 
                </div> 
              </button> 
            ))} 
          </div> 
          ...
          <div className="flex items-center justify-between pt-1"> 
            <p className="text-xs text-gx-fg4">{filtered.length} trade{filtered.length !== 1 ? "s" : ""} · page {page} of {totalPages}</p> 
